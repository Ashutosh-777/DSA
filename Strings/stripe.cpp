...
vector<vector<string>> parseNetworkCharegebackInfo(vector<string> &rows,unordered_map<string,double> &mMCurr,unordered_map<string,int> &getIndex){
...
string joinData(vector<string> &parsedResult,unordered_map<string,double> &mMCurr){
    string data = "";
    data += parsedResult.back() + parsedResult[0]+",";
    data += parsedResult[1]+",";
    string currency = parsedResult[3];
    double amt = stod(parsedResult[2])/mMCurr[currency];
    stringstream ss;
...
int main(){
    ...
    vector<vector<string>> parsedResults = parseNetworkCharegebackInfo(rows,mMCurr,getIndex);
    map<string,int> withdrawn;
    for(auto parsedResult:parsedResults){
        if(parsedResult[5]=="withdrawn"){
            string key = parsedResult.back();
            key+='-';
            key+=parsedResult[0];
            withdrawn[key] = 1;
        }
    }
    for(vector<string> parsedResult:parsedResults){
        string key = parsedResult.back();
        key+='-';
        key+=parsedResult[getIndex["transaction"]];
